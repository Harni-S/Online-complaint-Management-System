...
if($conn)
{
	$query1="SELECT * FROM `complaints`  WHERE `subcategory` = 'Panchayat'";
...
<i class="fa fa-reorder" onclick="callsh()" style="cursor:pointer;font-size:24px;margin:20px 2px 0px 20px;color:white"></i>
		...
		<img src="images/admin.png" style="border-radius:15%;margin-left:25%;float:left" width="120px" height="140px">
				<h3 style="float:left;cursor:pointer;margin:10px 2px 0px 20px;">CMS | Panchayat Admin</h3>
						
		</div>
				<div style="width:50%" >
				<a class="btn btn-danger" style="float:right;padding-left:30px;padding-right:30px;margin:10px;" href="panchayatadminlogin1.html">Logout</a>
		</div>
							...
							while($row=mysqli_fetch_assoc($result))
								{
									...
									if($cme=='1')
									{
										...
										$num++;
											...
											<form method='POST' action='panchayatsolvepend1.php'>
							...
							while($row=mysqli_fetch_assoc($result))
								{				
										...
										if($id==$inid)
										{
											...
											<form method='POST' action='panchayatsolveinpro1.php'>
											<input type='text' name='user' value='$usr' class='now'>
											<input type='text' name='compid' value='$id' class='now'>
