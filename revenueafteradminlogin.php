...
if($conn)
{
	$query1="SELECT * FROM `complaints`WHERE `subcategory` = 'Revenue'";
...
<html>
  ...
  <body>
		...
		<nav class="navbar-nav navbar-expand-sm bg-info" style="margin:0px;padding:0px;height:150px">
...
<i class="fa fa-reorder" onclick="callsh()" style="cursor:pointer;font-size:24px;margin:20px 2px 0px 20px;color:white"></i>
		...
		<img src="images/admin.png" style="border-radius:15%;margin-left:25%;float:left" width="120px" height="140px">
				<h3 style="float:left;cursor:pointer;margin:10px 2px 0px 20px;">CMS | Revenue Admin</h3>
						
		</div>
				<div style="width:50%" >
				<a class="btn btn-danger" style="float:right;padding-left:30px;padding-right:30px;margin:10px;" href="revenueadminlogin1.html">Logout</a>
		</div>
							...
							while($row=mysqli_fetch_assoc($result))
								{
									...
									if($cme=='1')
									{
										...
										$num++;
											...
											<form method='POST' action='revenuesolvepend1.php'>
											...
											echo "<a href='$fil' target='_blank'>view file</a><br />";
											 ...
											 echo "
											 Complaint-Subcategoty:   <span id='span'>$subcategory</span><br />
							...
							while($row=mysqli_fetch_assoc($result))
								{				
										...
										if($id==$inid)
										{
											...
											<form method='POST' action='revenuesolveinpro1.php'>
											<input type='text' name='user' value='$usr' class='now'>
											<input type='text' name='compid' value='$id' class='now'>
											...
											echo "<a href='$fil' target='_blank'>view file</a><br />";
											 ...
											 echo "
											 Complaint-Subcategory:   <span id='span'>$subcategory</span><br />
											 Complaint-Nature:        <span id='span'>$nat</span><br />
											 Complaint-Date:          <span id='span'>$da</span><br />
											 COMPLAINT >               <span id='span'>$co</span><br /></h6><br /><hr />";
										}
